...
export default class Question extends Flux.View {
	...
	loadQuestion(questionId) {
		const question = QuizStore.getQuestion(
			this.props.match.params.quiz_id,
			questionId
		);
		if (!question)
			window.location.href = "/" + this.props.match.params.quiz_id;
		const totalQuestions = QuizStore.getQuestionCount(
			this.props.match.params.quiz_id
		);
		this.timeLimit = QuizStore.getTimeLimit(
			this.props.match.params.quiz_id
		);
	...
	componentWillMount() {
		const questionId =
			this.props.match.params.question_id === "start"
				? 0
				: parseInt(this.props.match.params.question_id);
	...
	componentDidMount() {
		this.timeoutTimer = setInterval(() => {
			...
			if (this.timeLimit && this.timeLimit < this.state.seconds)
				this.timeIsUp();
		}, 1000);
	}
	...
	getCorrectAnswer() {
		...
		if (ans.length === 1) return ans[0];
			...
			alert(
				"Ooops! It seems that this question has more than one successfull answer, please report this to your teacher"
			);
	}
		...
		this.setState({
			...
			result: {
				...
				message: answer.correct
					? QuizStore.getRandom("correct")
					: QuizStore.getRandom("incorrect")
			},
			rightQuestions: answer.correct
				? this.state.rightQuestions + 1
				: this.state.rightQuestions
		});

		this.nextQuestionTimer = setTimeout(() => {
			if (this.state.isLastQuestion)
				this.setState({ totalResults: true });
	...
	loadNextQuestion() {
		...
		this.props.history.push(
			"/" + this.props.match.params.quiz_id + "/" + nextQuestionId
		);
	...
	printTotalResults() {
		return (
			<div className="text-center p-5 results">
				You had {this.state.rightQuestions} right questions out of{" "}
		...
		return (
			<div className="text-center p-5">
				{this.state.result ? (
					this.state.result.correct ? (
						...
						<p className="answer incorrect">
							{this.state.result.message}
							<p>{this.state.correctAnswer.option}</p>
						</p>
					)
				) : (
					<span>
				...
				<div className="breadcrumb">
					{parseInt(this.state.questionId) + 1} /{" "}
					{this.state.totalQuestions}
				</div>
			</div>
		);
